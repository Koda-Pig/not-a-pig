...
- Get all of the background images to render, not just the first one.
- Do not change anything else. Refer to this similar example.
...
class Game {
  ...
  thePlatformTheStickHits = () => {
    ...
    if (lastStick.rotation != 90)
      throw Error(`Stick is ${lastStick.rotation}°`);
  ...
  generateTree = () => {
    ...
    const x =
      furthestX + minGap + Math.floor(Math.random() * (maxGap - minGap));
  ...
  loadSoundEffects = () => {
    // Stretching sound
    this.soundEffects.stretching = new Audio(
      "./sound-effects/cartoon-rise.wav"
    );
